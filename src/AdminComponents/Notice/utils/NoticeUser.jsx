    ...
    <div className="bg-white p-4 rounded-lg shadow-md w-full flex flex-col">
      ...
      <div className="flex justify-between mobile:max-tablet:flex-col mobile:max-laptop:gap-2">
        <div className="flex gap-10  mobile:max-tablet:whitespace-nowrap mobile:max-laptop:gap-2 overflow-auto">
          <button className={`p-2 mx-1 ${selectedRole === 'all' ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600"}`}
          ...
          <button className={`p-2 mx-1 ${selectedRole === 'class' ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600"}`}
            onClick={() => selectRole('class')}>Class</button>
          <button className={`p-2 mx-1 ${selectedRole === 'subAdmin' ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600"}`}
            onClick={() => selectRole('subAdmin')}>Sub Admin</button>
        </div>
        <div className="flex justify-between mobile:max-tablet:gap-2">
          <div><button className="bg-blue-200 hover:bg-blue-500 font-bold py-2 px-4 rounded mobile:max-tablet:py-0 mobile:max-tablet:px-2" onClick={handleOpenModal}>Write Notice</button></div>
        </div>
      </div>
