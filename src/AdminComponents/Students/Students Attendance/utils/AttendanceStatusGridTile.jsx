...
export default function AttendanceStatusGridTile({ data, month }) {
    ...
    return (
        <motion.div 
      className="flex mb-2 bg-blue-100 p-4 rounded-lg shadow-lg"
      ...
      <div className="flex w-full h-full overflow-x-auto mt-3 items-start">
        {dateRange.map((date, index) => (
          ...
          >
              ...
              {index === 0 &&
                students.map((student, index2) => (
                  ...
                  >
                    {index2 === 0 ? (
                      <div>
                        <h1 className="font-semibold h-12 text-xl border border-blue-300 px-2 pt-2 bg-blue-200 rounded-t-lg">Students</h1>
                        <motion.h1 
                          className={`font-normal text-lg h-12 text-start whitespace-nowrap border border-blue-300 px-2 pt-2 cursor-pointer ${selectedStudent === index2 ? 'bg-blue-300' : "bg-blue-200"}`}
            ...
            <motion.div 
              className={`flex flex-col justify-start items-center h-12 ${selectedDate === index ? "bg-blue-300" : "bg-blue-200"}`}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <motion.div 
                className="flex flex-col w-12 items-center h-12 border border-blue-300 cursor-pointer" 
                onClick={() => handleDateClick(index)}
                whileHover={{ backgroundColor: "#D8B4FE" }}
              >
                <span className="font-semibold text-lg">{format(date, 'd')}</span>
                <span className={`text-sm text-blue-700`}>{format(date, 'EEE')}</span>
              </motion.div>
              <div className={`flex flex-col justify-center`}>
                {students.map((student, index2) => (
                  <motion.div 
                    key={index2} 
                    className={`flex justify-center font-normal text-lg w-12 h-12 whitespace-nowrap border border-blue-300 px-2 pt-3 ${selectedDate === index ? "bg-blue-300" : ""} ${selectedStudent === index2 ? "bg-blue-300" : ""} ${selectedStudent === index2 && selectedDate === index ? "bg-blue-400" : ""}`}
