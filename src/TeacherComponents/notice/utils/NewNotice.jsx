...
function NewNotice({ setShowModal }) {
  ...
  return (
    <AnimatePresence >
      <motion.div
        className="fixed z-50 inset-0 flex items-center mobile:max-tablet:justify- mobile:max-tablet: mobile:max-tablet: justify-center w-full bg-gray-800 bg-opacity-50 overflow-y-auto"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      >
        <motion.div
          className="bg-white rounded-lg mx-2 shadow-xl w-1/2 mobile:max-tablet:w-auto my-10"
