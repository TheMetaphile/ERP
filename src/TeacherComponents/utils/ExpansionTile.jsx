...
function ExpansionTile({ title, childrens, image, alternateText, route, openInNewTab = false }) {
  ...
  return (
    <div className={`${expanded ? 'bg-secondary rounded-xl px-2 py-3 mb-2' : 'px-2 py-3'}`}>
      {childrens.length > 0
        ...
        (
          openInNewTab ? (
            <div className="flex justify-start items-center cursor-pointer" onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
