...
export default function ClassTeacherOnLeaveRow({ Teacher, index, date, session }) {
    ...
    const [originalSubject, Substitute] = useState();
    ...
    const handleSuggestionClick = (suggestion, subject, lectureNo, classs, section) => {
        console.log(suggestion, subject, lectureNo, classs, section, Teacher.email);
        SubstituteTeacher(suggestion, subject, lectureNo, classs, section,);
        setShowSuggestions(false);
    ...
    return (
        <>
            {Teacher.lecturesForTheDay.map((data, indexx) => {
                ...
                return (
                    <tr key={indexx} className="border-b border-gray-200 last:border-none">
                        ...
                        <td className="py-3 px-6 text-center items-center whitespace-nowrap">
                            {substitutionDetail ? (
                                <div className="flex justify-start gap-2 items-center">
                                    <img src={substitutionDetail.profileLink} alt="img" className="rounded-full h-12 w-12" />
                                    <div className="text-start">
                                        <p>{substitutionDetail.name}</p>
                                        {substitutionDetail.employeeId}
                                    </div>
                                </div>
                            ) : editingRow ? (
                                <div>
                                        ...
                                        <ul className="absolute z-10 bg-white border rounded-md mt-1 max-h-70 overflow-y-auto" ref={suggestionsRef}>
                                            {suggestions.map((suggestion, idx) => (
                                                <li
                                                    ...
                                                    onClick={() => handleSuggestionClick(suggestion, data.subject, data.lectureNo, data.class, data.section)}
                                                >
