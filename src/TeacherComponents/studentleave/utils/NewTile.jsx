        ...
        >
            {data.length > 0 ? (
                data.filter(student => student.status === "Pending").map((student, studentIndex) => (
                    <motion.div
                        key={studentIndex}
                        className={`border border-blue-200 p-4 rounded-lg shadow-md ${bgClass} ${textClass} hover:shadow-lg transition-shadow duration-300`}
                        ...
                        <AnimatePresence>
                            {expanded === `${studentIndex}` && (
                                ...
                                >
                                    ...
                                    <p className={`text-gray-700 ${darkMode ? 'bg-gray-800' : 'bg-blue-100'} ${textClass} p-3 rounded-lg`}>{student.reason}</p>
            ...
            {data.filter(student => student.status === 'Approved').map((student, studentIndex) => (
                <motion.div
                    key={student._id}
                    className={`border border-green-200 p-4 rounded-lg shadow-md ${bgClass} ${textClass} hover:shadow-lg transition-shadow duration-300`}
                    whileHover={{ scale: 1.02 }}
                    layout
                >
                    <div className="flex justify-between items-center cursor-pointer mobile:max-tablet:flex-col mobile:max-tablet:items-start mobile:max-tablet:gap-2" onClick={() => handleClick(studentIndex)}>
                    ...
                    <AnimatePresence>
                        {expanded === studentIndex && (
                            ...
                            >
                                ...
                                <p className={`text-gray-700 ${darkMode ? 'bg-gray-800' : 'bg-green-100'} ${textClass} p-3 rounded-lg`}>{student.reason}</p>
                                <div className="mt-4">
            ...
            {data.filter(student => student.status === 'Rejected').map((student, studentIndex) => (
                <motion.div
                    key={student._id}
                    className={`border border-red-200 p-4 rounded-lg shadow-md ${bgClass} ${textClass} hover:shadow-lg transition-shadow duration-300`}
                    whileHover={{ scale: 1.02 }}
                    layout
                >
                    <div className="flex justify-between items-center cursor-pointer mobile:max-tablet:flex-col mobile:max-tablet:items-start mobile:max-tablet:gap-2" onClick={() => handleClick(studentIndex)}>
                    ...
                    <AnimatePresence>
                        {expanded === studentIndex && (
                            ...
                            >
                                ...
                                <p className={`text-gray-700 ${darkMode ? 'bg-gray-800' : 'bg-red-100'} ${textClass} p-3 rounded-lg`}>{student.reason}</p>
                                <div className="mt-4">
