  ...
  return (
    <div className={`w-screen h-screen flex overflow-x-auto no-scrollbar pt-20 mobile:max-tablet:mt-16 transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-black'}`}>
      ...
      <div className="flex flex-grow relative">
        ...
        >
          <SubadminDrawer isOpen={isDrawerOpen} darkMode={darkMode} />
        </div>

        {/* Main Content */}
        <div className={`flex-grow overflow-auto no-scrollbar transition-all duration-300 ${
          isDrawerOpen ? 'mobile:max-tablet:ml-64' : 'ml-0'
        } ${
        ...
        <div 
          className={`fixed right-0 top-20 z-30 h-[calc(100vh-5rem)]
            ...
            ${darkMode ? 'bg-gray-800' : 'bg-white'}`}
        >
          <SubAdminEndDrawer darkMode={darkMode} />
        </div>
      </div>
      ...
      {showNotification && notification && (
        <div 
          className={`fixed bottom-4 right-4 z-50 p-4 rounded-lg shadow-lg max-w-xs transform transition-all duration-300 ease-in-out ${
