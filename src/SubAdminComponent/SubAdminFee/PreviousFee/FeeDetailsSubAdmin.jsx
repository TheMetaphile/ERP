...
function PreviousFeeDetailsSubAdmin() {
    ...
    return (
        ...
        <div className={`flex flex-col px-6 mobile:max-tablet:p-2 py-8 ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
            <ToastContainer />
            <div className={`flex justify-between items-center mb-8 mobile:max-tablet:flex-col ${darkMode ? 'text-blue-300' : 'text-blue-500'}`}>
                <h1 className="text-3xl font-bold flex items-center mobile:max-tablet:text-lg whitespace-nowrap">
                    <MdSchool className="mr-2" />Previous Fee Details
                </h1>
            </div>
            <div className=' overflow-auto'>
                <div className='overflow-auto w-full'>
                    <div className={`flex justify-between py-2 rounded-t-lg border border-b-2 whitespace-nowrap mobile:max-tablet:w-fit ${darkMode ? 'bg-gray-700 text-gray-200' : 'bg-blue-200'}`}>
                    ...
                    {loading ? (
                        ...
                        details.length > 0 ? (
                            <div className=' mobile:max-tablet:w-fit overflow-auto'>
                                {details.map((details, index) => (
                                    <motion.div
                                        ...
                                        className={`border-b ${darkMode ? 'border-gray-700 hover:bg-gray-800' : 'border-gray-200 hover:bg-gray-50'} transition-colors`}
                                    >
                                        <Link to={`/Sub-Admin/StudentsFee/PreviousFeeSubAdmin/${details.Student.email}?Class=${details.Student.currentClass}&session=${details.session}&name=${details.Student.name}&section=${details.Student.section}`}>
                                            <div
                                                key={index}
                                                className={`px-1 flex justify-between w-full py-2 pl-2 h-fit border gap-x-4 items-center 
                                            ${clickedIndex === index ? (darkMode ? 'bg-gray-700' : 'bg-secondary') : (darkMode ? 'bg-gray-900' : 'bg-white')}
                                        `}
                            ...
                            <div className={`text-center mt-2 ${darkMode ? 'text-gray-400' : ''}`}>
