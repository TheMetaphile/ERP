...
export default function DoughnutSecond({ chartData, title }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col h-full w-full mobile:max-tablet:w-full justify-evenly items-center shadow-lg bg-white border border-purple-300 rounded-lg px-4 py-6"
    >
      <motion.div
        initial={{ y: -20 }}
        animate={{ y: 0 }}
        ...
        className="w-11/12"
      >
        <h2 className="mb-4 whitespace-nowrap mobile:max-tablet:text-xl text-2xl text-center font-semibold text-purple-800 flex items-center justify-center">
      ...
      >
        {chartData.labels.map((label, index) => (
          <motion.div
            ...
            className={`text-center ${chartData.datasets[0].bg[index]} text-xl mr-3 bg-purple-200 rounded-lg p-2`}
          >
